...
def split(lang, dataset, valid_len):
    ...
    print("remove valid duplicates")
    with open(tmp + "valid-cleaned.tags." + lang + ".tok.sl", "r", encoding="utf-8") as fsl, \
      open(tmp + "valid-cleaned.tags." + lang + ".tok.en", "r", encoding="utf-8") as fen:
        for rsl, ren in zip(fsl, fen):
            if "http" not in rsl and "http" not in ren and rsl not in sl_unique_valid and ren not in en_unique_valid:
                # remove duplicate lines (applause, etc.), and lines starting with http
                sl_unique_valid.append(rsl)
                en_unique_valid.append(ren)
    ...
    with open(tmp + "train.sl", "w", encoding="utf-8") as fsl_train, \
    ...
    with open(tmp + "valid.sl", "w", encoding="utf-8") as fsl_valid, \
        open(tmp + "valid.en", "w", encoding="utf-8") as fen_valid:
        for row in sl_unique_valid:
            fsl_valid.write(row)
        for row in en_unique_valid:
            fen_valid.write(row)
